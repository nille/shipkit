...
class KiroCompiler(Compiler):
    ...
    def compile(self, ctx: CompileContext, dry_run: bool = False) -> CompileResult:
        ...
        # Compile: guidelines (with discovery instructions), subagents, MCP, hooks, agent config
        for step in [self._compile_guidelines, self._compile_subagents, self._compile_mcp, self._compile_hooks, self._compile_agent]:
...
"""
    ...
    def _compile_agent(self, ctx: CompileContext, dry_run: bool) -> tuple[list, list, list]:
        ...
        from shipkit.compilers.agents import write_kiro_agent

        written, skipped, warnings = [], [], []

        agent_file = write_kiro_agent(ctx, dry_run=dry_run)
        if agent_file:
            if dry_run:
                written.append(".kiro/agents/shipkit.json (dry-run)")
            else:
                written.append(".kiro/agents/shipkit.json")
