...
2. **User global** (`<home>/steering/`, `<home>/skills/`) — personal additions and overrides
3. **Project** (`<home>/projects/<name>/`) — project-specific overrides

Higher layers override lower layers. A steering rule in the user layer with the same filename as a package rule replaces it entirely. A skill with the same directory name replaces it.

## User Content Principle

The shipkit home directory (`~/.config/shipkit/`) is exclusively for user content. Core steering rules and skills live in the shipkit package and are never copied there. This means:
...
To add a personal steering rule:
1. Create a `.md` file in `<home>/steering/`
...
Each registered project can have its own steering and skills in `<home>/projects/<name>/`:
- `steering/` — project-specific rules (e.g., "this project uses PostgreSQL")
