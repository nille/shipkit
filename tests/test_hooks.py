...
class TestHookConfig:
    ...
    def test_resolve_home_path(self, initialized_home, monkeypatch):
        ...
        from shipkit.core.hooks.lib.config import resolve_home_path
    ...
    def test_resolve_project_name(self, tmp_path):
        ...
        from shipkit.core.hooks.lib.config import resolve_project_name
    ...
    def test_is_hook_session(self, monkeypatch):
        from shipkit.core.hooks.lib.config import is_hook_session
...
class TestTranscript:
    ...
    def test_extract_turns(self, jsonl_transcript):
        from shipkit.core.hooks.lib.transcript import extract_turns
    ...
    def test_count_user_turns(self, jsonl_transcript):
        from shipkit.core.hooks.lib.transcript import extract_turns, count_user_turns
        turns = extract_turns(jsonl_transcript)
        assert count_user_turns(turns) == 3

    def test_generate_title(self, jsonl_transcript):
        from shipkit.core.hooks.lib.transcript import extract_turns, generate_title
        turns = extract_turns(jsonl_transcript)
    ...
    def test_build_summary(self, jsonl_transcript):
        from shipkit.core.hooks.lib.transcript import extract_turns, build_summary
        turns = extract_turns(jsonl_transcript)
...
class TestSessionContext:
    ...
    def test_format_session_context_empty(self, initialized_home, monkeypatch):
        monkeypatch.setenv("SHIPKIT_HOME", str(initialized_home))
        from shipkit.core.hooks.lib.session_context import format_session_context
    ...
    def test_format_pending_retro_nudge_empty(self, initialized_home, monkeypatch):
        monkeypatch.setenv("SHIPKIT_HOME", str(initialized_home))
        from shipkit.core.hooks.lib.session_context import format_pending_retro_nudge
        result = format_pending_retro_nudge()
        assert result == ""

    def test_format_learned_preferences_empty(self, initialized_home, monkeypatch):
        monkeypatch.setenv("SHIPKIT_HOME", str(initialized_home))
        from shipkit.core.hooks.lib.session_context import format_learned_preferences
...
class TestDebounce:
    ...
    def test_get_debounce_dir(self, initialized_home, monkeypatch):
        monkeypatch.setenv("SHIPKIT_HOME", str(initialized_home))
        from shipkit.core.hooks.lib.debounce import get_debounce_dir
    ...
    def test_cleanup_debounce(self, initialized_home, monkeypatch):
        monkeypatch.setenv("SHIPKIT_HOME", str(initialized_home))
        from shipkit.core.hooks.lib.debounce import get_debounce_dir, cleanup_debounce
